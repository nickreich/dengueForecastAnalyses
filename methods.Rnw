...
print(xtable(biweek_table, digits=0, align="cccccc", 
             caption="Map of Julian days to biweeks used in data aggregation."), 
      include.rownames=FALSE, caption.placement="top")
@


A generic biweek $b_k$ is defined as an interval $[t_{k}, t_{k+1})$ where $t_k$ is the time where the biweeks begins (e.g. Jan 1, 00h00m00s) and $t_{k+1}$ is the time when the biweeks ends (e.g. Jan 14, 11h59m59s). Every dataset is divided up into $N$ bi-weeks ($b_1$ through $b_{N}$), each of either 14 or 15 days (see Table \ref{tab:biweekMap}). 
