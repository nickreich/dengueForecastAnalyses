...
load(file.path(aggregated_data_dir, 'standard_wide_format', fname))
...
colnames(outbreak.prob) <- paste("week", CURRENT_WEEK-seq(8, -2, by=-2))
@
...
#colnames(outbreak_prob_by_region) <- c("region", paste("week", CURRENT_WEEK-seq(8, -2, by=-2)))
...
\begin{center}
\begin{tabular}{rl}
Data received for: & Week \Sexpr{CURRENT_WEEK} (biweek \Sexpr{CURRENT_WEEK/2}) \\
Data assumed complete through: & Week \Sexpr{CURRENT_WEEK-8} (biweek \Sexpr{(CURRENT_WEEK-8)/2}) \\
Forecasts made for: & Biweeks 
\Sexpr{(CURRENT_WEEK-8)/2+1}, 
\Sexpr{(CURRENT_WEEK-8)/2+2}, 
\Sexpr{(CURRENT_WEEK-8)/2+3}, 
\Sexpr{(CURRENT_WEEK-8)/2+4}, 
\Sexpr{(CURRENT_WEEK-8)/2+5}, 
\Sexpr{(CURRENT_WEEK-8)/2+6} \\
\end{tabular}
\end{center}
\label{default}
\end{table}%
...
for(prov in 1:nrow(dat)){
        ...
        plot.prediction(prov, den_forecast, ylim=c(0, top), xlim=c(start, 2014+(CURRENT_WEEK+3)/52), lwd=2, xaxt="n")
        ...
        abline(v=2014+(CURRENT_WEEK-8)/52, lty=2, col="gray", lwd=2)
        abline(v=2014+CURRENT_WEEK/52, lty=2, col="gray", lwd=2)
